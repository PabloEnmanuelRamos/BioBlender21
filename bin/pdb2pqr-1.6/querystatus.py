...
def createresults(header, input, name, time, missedligands=None):
    ...
    file.write("here</a>.<p>\n")
    file.write(
        "if you would like to run apbs with these results, please click <a "
        "href=\"%s../apbs/index.py?pdb2pqr-id=%s\">here</a>.<p>\n" % (
            WEBSITE[:-1], name))
...
def mainCGI():
    ...
    """
    global filelist, runtime, starttime, jobid, opal_url, apbs_input, resultsurl, typemap, pqrOpalFileLength, logff, REMOTE_ADDR, resp
    logopts = {}
    ...
    if have_opal and progress is None:
    ...
    if progress == "complete":
        ...
        if calctype == "pdb2pqr":
            if have_opal:
                ...
                if os.path.isfile('%s%s%s/pdb2pqr_opal_log' % (INSTALLDIR, TMPDIR, form["jobid"].value)):
                    pdb2pqrOpalLogFile = open('%s%s%s/pdb2pqr_opal_log' % (INSTALLDIR, TMPDIR, form["jobid"].value),
                                              'r')
                ...
                for i in range(0, len(filelist)):
                    if filelist[i]._name[-7:] == ".propka" or (
                            filelist[i]._name[-13:] == "-typemap.html" and typemap == True) or filelist[i]._name[
                                                                                               -4:] == ".pqr" or \
                            filelist[i]._name[-3:] == ".in":
                        if filelist[i]._name[-4:] == ".pqr":
                            # Getting pqr file length for PDB2PQR Opal run
                            f = url.urlopen(filelist[i]._url)
                ...
                # for extension in ["-typemap.html", ".pqr", ".in"]: print ("<li><a href=%s%s%s/%s%s>%s%s</a></li>" %
                # (WEBSITE, TMPDIR, jobid, jobid, extension, jobid, extension))
        elif calctype == "apbs":
            if have_opal:
                for i in range(0, len(filelist)):
                    if filelist[i]._name[-3:] == ".dx":
                        ...
                        url.urlretrieve(filelist[i]._url,
                                           '%s%s%s/%s' % (INSTALLDIR, TMPDIR, zipjobid, filelist[i]._name))
                        ...
                        print("<li><a href=%s%s%s/%s>%s</a></li>" % (
                            WEBSITE, TMPDIR, zipjobid, outputfilezip, outputfilezip))
            else:
                ...
                for outputfile in outputfilelist:
                    ...
                    print("<li><a href=%s%s%s/%s>%s</a></li>" % (
                        WEBSITE, TMPDIR, jobid, os.path.basename(outputfilezip), os.path.basename(outputfilezip)))
        ...
        print("<HR>")
        print("<small>Your results will appear at <a href=%s>this page</a>. If you want, you can bookmark it and "
              "come back later (note: results are only stored for approximately 12-24 hours).</small>" % resultsurl)
    elif progress == "version_mismatch":
        print(
            "The versions of APBS on the local server and on the Opal server do not match, so the calculation could "
            "not be completed")
...
if __name__ == "__main__" and ("REQUEST_METHOD" in os.environ):
    ...
    if not ("jobid" in form) and form["calctype"].value == "pdb2pqr":
    ...
    if (form["calctype"].value == "pdb2pqr" and HAVE_PDB2PQR_OPAL == "1") or (
            form["calctype"].value == "apbs" and APBS_OPAL_URL != ""):
        have_opal = True
