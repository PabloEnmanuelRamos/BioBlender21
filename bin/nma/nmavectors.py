...
def usage():
    print(
        "Compute a normal mode analysis based on Anistropic Network Model (ANM) on alpha carbon, according to minimized input pdb struture (local file or pdb code) and provide an output pdb structure with normal mode vectors on CA instead of atom positions for the specified normal mode")
    print("python nmavectors.py --input=input.pdb -output=output.pdb -mode=mode [cutoff=][--gamma]")


def main():
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hi:o:m:g:c:",
                                   ["help", "input=", "output=", "mode=", "gamma=", "cutoff="])
